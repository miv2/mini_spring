...
public class ChatMessageController {
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "403", description = "참여 권한 없음")
    })
    public ApiResult<ChatMessageSliceResponse> getMessages(
        ...
        return ApiResult.success(chatMessageService.getMessages(roomId, currentUserId(memberAdapter), cursor, size));
    }
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "성공"),
            @ApiResponse(responseCode = "400", description = "유효하지 않은 메시지"),
            @ApiResponse(responseCode = "401", description = "인증 필요")
    })
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "400", description = "삭제 가능 시간 초과"),
            @ApiResponse(responseCode = "401", description = "인증 필요")
    })
